...
serve(async (req) => {
  ...
  try {
    ...
    // For images and PDFs, use the AI model with vision/document understanding
    ...
    const messages: any[] = [
      ...
      },
      {
        ...
        content: [
          ...
          {
            type: "text",
            text: `Extract all text content from this ${isImage ? "image" : "document"} file "${fileName || "document"}". Return only the extracted text.`,
          },
        ],
      },
    ];
