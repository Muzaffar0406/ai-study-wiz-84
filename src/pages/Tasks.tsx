...
const TasksContent = () => {
  ...
  const processedTasks = useMemo(() => {
    ...
    if (filterSubject !== "all") filtered = filtered.filter(t => t.subject === filterSubject);
    ...
    filtered.sort((a, b) => {
      switch (sortBy) {
        ...
        case "due_time": return (a.due_time || "99:99").localeCompare(b.due_time || "99:99");
  ...
  const clearFilters = () => { setFilterPriority("all"); setFilterSubject("all"); };
      ...
      <PageContent className="space-y-6">
            ...
            <div className={`flex ${isMobile ? "flex-col gap-3" : "items-center gap-4 flex-wrap"}`}>
              ...
              <div className="flex items-center gap-2">
                ...
                <Select value={filterPriority} onValueChange={(v) => setFilterPriority(v as FilterPriority)}>
                  <SelectTrigger className="w-[140px] h-9 text-xs"><SelectValue placeholder="Priority" /></SelectTrigger>
                  <SelectContent>
              ...
              {hasActiveFilters && (
                <button onClick={clearFilters} className="flex items-center gap-1 text-xs text-primary hover:text-primary/80 font-medium transition-colors ml-auto">
        ...
        {loading ? (
          ...
          <div className="bg-card rounded-2xl p-10 text-center shadow-[var(--shadow-soft)]">
            ...
            <button onClick={clearFilters} className="text-sm text-primary hover:underline mt-2">Clear all filters</button>
          </div>
        ) : (
          <>
            {incompleteTasks.length > 0 && (
              <div className="space-y-3">
                <h2 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Pending ({incompleteTasks.length})</h2>
                <div className="space-y-2">
                  {incompleteTasks.map(task => (
                    <TaskCard key={task.id} id={task.id} title={task.title} subject={task.subject} priority={task.priority} dueDate={task.due_date || undefined} dueTime={task.due_time || ""} completed={task.completed} onToggle={handleToggle} onDelete={handleDelete} />
                  ))}
                </div>
              </div>
            )}
            {completedTasks.length > 0 && (
              <div className="space-y-3">
                <h2 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">Completed ({completedTasks.length})</h2>
